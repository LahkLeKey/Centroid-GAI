 ...
 * vocabulary bounds. Callers must supply an appropriate index or use the invalid sentinel
 * deliberately.
 *
 ...
 */
static inline int
cgai_token_id_is_valid(cgai_token_id id) { /* Step 1: Distinguish an ordinary index from the
                                              reserved lookup-failure sentinel. */
